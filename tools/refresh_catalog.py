...
def clean_name(value: str) -> str:
    return value.replace("\N{WARNING SIGN}", "").replace("\N{NO-BREAK SPACE}", " ").strip()
...
def as_percent(threshold: Decimal | None, base: Decimal | None) -> int:
    if threshold is None or base is None or base == 0:
        return 0
    return int(
        ((threshold * 100) / base).to_integral_value(rounding=ROUND_HALF_UP)
    )
...
def build_catalog(
    ...
    with ThreadPoolExecutor(max_workers=jobs) as executor:
        futures = {
            executor.submit(fetch_facts, row[0]): row[0]
            for row in rows
        }
