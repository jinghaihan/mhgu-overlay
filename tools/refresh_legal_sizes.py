...
def legal_percentages(base_size_x100: int, sizes: set[Decimal]) -> set[int]:
    ...
    return {
        int(
            ((size * Decimal(100)) / base_size).to_integral_value(
...
def build_ranges(timeout: float, retries: int, jobs: int) -> dict:
    ...
    with ThreadPoolExecutor(max_workers=jobs) as executor:
        futures = {
            executor.submit(fetch_range, monster, timeout, retries): monster
    ...
    validation_counts = {
        state: sum(
            size_range["validation"] == state
            for size_range in ranges.values()
        )
        for state in (
            "mhcrown-matched",
            "mhcrown-mismatch",
