...
CoreOutput Engine::update(
    ...
    output.locale = resolve_locale(
        snapshot.game,
        settings.locale_mode,
        snapshot.detected_locale
    );
    ...
    for (std::size_t index = 0; index < count; ++index) {
        ...
        view.hp_percent_x10 = static_cast<std::uint16_t>(
            std::min<std::uint64_t>(
                1000U,
                (static_cast<std::uint64_t>(source.hp) * 1000U) /
                    source.max_hp
            )
        );
        view.size_percent = source.size_percent;
        view.actual_size_x100 = actual_size_x100(
            *definition,
            source.size_percent
        );
        ...
        if (!settings.size_lock_armed ||
            settings.size_preset == SizePreset::Off ||
            !definition->variable_size) {
            continue;
        }

        const auto target = size_percent_for_preset(
            *definition,
